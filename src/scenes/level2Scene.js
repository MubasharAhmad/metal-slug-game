...
export default class Level2Scene extends Phaser.Scene {
    ...
    addBackground() {
        // Add background images for the sky and sun
        for(let i = 0; i < 3; i++) {
            const bg_sky = this.add.image(this.game.config.width / 2, this.game.config.height / 2, "level2_sky");
            bg_sky.setScale(0.65);
            bg_sky.setX(this.game.config.width / 2 + (bg_sky.displayWidth * i));
        }
        const bg_sun = this.add.image(this.game.config.width / 2, this.game.config.height / 2, "level2_sun");
        bg_sun.setScale(0.65);

        // Add background images for the ruins and houses
        for(let i = 0; i < 3; i++) {
            const bg_ruins = this.add.image(this.game.config.width / 2, this.game.config.height / 2, "level2_ruins");
            bg_ruins.setScale(0.65);
            bg_ruins.setX(this.game.config.width / 2 + (bg_ruins.displayWidth * i));

            const bg_houses3 = this.add.image(this.game.config.width / 2, this.game.config.height / 2, "level2_houses3");
            bg_houses3.setScale(0.65);
            bg_houses3.setX(this.game.config.width / 2 + (bg_houses3.displayWidth * i));

            const bg_houses2 = this.add.image(this.game.config.width / 2, this.game.config.height / 2, "level2_houses2");
            bg_houses2.setScale(0.65);
            bg_houses2.setX(this.game.config.width / 2 + (bg_houses2.displayWidth * i));

            const bg_houses1 = this.add.image(this.game.config.width / 2, this.game.config.height / 2, "level2_houses1");
            bg_houses1.setScale(0.65);
            bg_houses1.setX(this.game.config.width / 2 + (bg_houses1.displayWidth * i));

            const bg_fence = this.add.image(this.game.config.width / 2, this.game.config.height / 2, "level2_fence");
            bg_fence.setScale(0.65);
            bg_fence.setX(this.game.config.width / 2 + (bg_houses1.displayWidth * i));
        }
    }

    addPlatforms() {
        // Create left boundary platform
        this.left_boundry = this.platforms.create(-50, this.game.config.height / 2 + 100, "level2_platform");
        ...
        this.right_boundry = this.platforms.create(3620, this.game.config.height / 2 + 100, "level2_platform");
        this.right_boundry.setAlpha(0);
        this.right_boundry.setSize(10, 500);
        this.right_boundry.platformName = "side_boundry";

        // Create ground platform
        this.ground = this.platforms.create(this.game.config.width / 2, this.game.config.height / 2, "level2_road");
        ...
        this.platform_1 = this.platforms.create(this.game.config.width / 2 + 600, this.game.config.height / 2 + 100, "level2_platform");
        this.platform_1.setScale(0.65);
        this.platform_1.refreshBody();

        this.platform_2 = this.platforms.create(this.game.config.width / 2 + 2000, this.game.config.height / 2 + 100, "level2_platform");
        this.platform_2.setScale(0.65);
        this.platform_2.refreshBody();

        // Add more ground images to extend the road
        for (let i = 1; i < 3; i++) {
            const g = this.add.image(this.game.config.width / 2, this.game.config.height / 2, "level2_road");
            g.setScale(0.65);
